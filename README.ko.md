...
`remove_path`는 파일을 휴지통으로 보내지 않고 영구 삭제합니다. `apply_patch`는 패치의 모든 대상 경로를 검사해 작업 폴더와 허용 프로젝트 루트 밖의 수정은 거부합니다. 삭제 도구는 특히 주의해서 사용하세요. 전체 도구 목록과 입력 형식은 [영문 README](README.md#available-tools)를 참고하세요.
