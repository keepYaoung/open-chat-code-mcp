...
> [!IMPORTANT]
> 이 MCP는 Codex 데스크톱 앱에만 추가해서는 안 됩니다. **ChatGPT 웹 앱**의 Plugins 또는 Apps 설정에서 등록해야 ChatGPT 웹 대화에서 도구를 사용할 수 있습니다. 웹에서 등록한 뒤에는 같은 ChatGPT 계정의 Codex 데스크톱 앱에서도 연결을 사용할 수 있으므로, 사용 전 활성화 상태를 확인하세요.
...
이 호스트를 쓰는 ChatGPT 계정 또는 프로젝트의 지침에 아래 문구를 저장합니다.

> 내가 이 채팅 세션에서 실행을 명시적으로 요청할 때만 Work, 에이전트 모드 또는 MCP 도구 실행을 시작해줘.

이 설정은 계획·설명·코드 리뷰를 기본적으로 일반 대화에 남게 합니다. ChatGPT 제품 정책을 우회하지는 않지만, 연결된 호스트를 실행하기 전에 사용자의 실행 의도를 명확하게 합니다.

이 호스트를 쓸 새 대화를 시작할 때는 첫 실행 요청에서 MCP 이름을 명시하세요. 예: `home-mac-coding-mcp를 사용해 연결된 Mac 프로젝트를 확인해줘.` 도구를 선택하기 전에 의도한 실행 호스트를 분명하게 합니다.
