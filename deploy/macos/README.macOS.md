...
- `MCP_MACOS_SANDBOX=true` wraps `exec_command` and `run_script` with `sandbox-exec` and fails closed when that legacy macOS executable is unavailable. `MCP_MACOS_SANDBOX_HOME_ACCESS=isolated` permits only the app-owned isolated `HOME`, never the user's normal home directory.
