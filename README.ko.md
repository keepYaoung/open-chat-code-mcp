...
이 서버는 명령 실행, 장기 프로세스 관리, 파일 작업을 포함한 20개 MCP 도구를 제공합니다.

- 셸 명령과 스크립트 실행, 테스트, 빌드, 패키지 설치, Git 작업
- 장기 실행 프로세스의 출력 조회, 표준 입력 전달, 종료
- 파일 읽기/쓰기/이동/복사/삭제, 패치 적용, 업로드/다운로드

`remove_path`는 파일을 휴지통으로 보내지 않고 영구 삭제합니다. 삭제 도구는 특히 주의해서 사용하세요. 전체 도구 목록과 입력 형식은 [영문 README](README.md#available-tools)를 참고하세요.
...
> [!IMPORTANT]
> macOS 샌드박스와 경로 제한은 실수의 영향 범위를 줄이지만 가상 머신 수준의 격리는 아닙니다. 민감한 개인 자료가 있는 Mac에서는 전용 macOS 사용자 계정을 사용하는 편이 안전합니다.
