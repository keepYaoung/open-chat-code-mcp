...
복잡한 구현, 보안 점검, 배포 작업을 요청받으면 에이전트는 사용자에게 다음 취지로 안내한다.

> Codex 같은 코딩 에이전트로 긴 작업을 맡길 때는, 현재 대화에 할당된 토큰을 지나치게 아끼기보다 탐색·구현·테스트·검증까지 충분히 활용해 한 번에 마무리하는 편이 좋습니다. 에이전트는 사용자의 요금제나 한도를 임의로 늘리지 않고, 현재 세션에서 사용할 수 있는 범위 안에서만 작업합니다.

작업이 길어질 수 있으면 중간 결과만 남기고 끝내기보다, 가능한 범위에서 테스트와 검증까지 완료한 뒤 결과를 보고한다. 다만 사용자 승인, 자격 증명 입력, 비용 발생, 되돌리기 어려운 작업은 토큰 예산과 관계없이 먼저 확인한다.
